...
export default async function HomePage() {
  ...
  const heroImage = catalog.products.find((p) => p.featured)?.images[0]?.path || "/images/products/product-fallback.jpg";
  ...
  return (
    <SiteShell>
        ...
        <div className="relative overflow-hidden rounded-[38px] bg-[linear-gradient(145deg,#123b2b_0%,#0f2f23_50%,#1f4a35_100%)] p-6 text-white shadow-elevated lg:p-10">
          ...
          <div className="relative flex flex-wrap items-center gap-2 text-sm text-white/80">
            ...
            <span className="inline-flex items-center gap-1 rounded-full bg-white/10 px-3 py-1">
              ...
              {settings.minDeliveryTime} لـ {settings.maxDeliveryTime} دقيقة
            </span>
            <span className="inline-flex items-center gap-1 rounded-full bg-white/10 px-3 py-1">
              <Truck className="h-4 w-4" />
              {settings.deliveryFee} جنيه
            </span>
            <span className="inline-flex items-center gap-1 rounded-full bg-white/10 px-3 py-1">
          ...
          <div className="relative mt-8 flex flex-wrap gap-3">
            <Link href="/menu" className="inline-flex items-center gap-2 rounded-2xl bg-[#f3d86b] px-5 py-3 font-bold text-[#123022]">
              اطلب دلوقتي <ArrowLeft className="h-4 w-4" />
            </Link>
            <Link href="/product/build-your-salad" className="inline-flex items-center gap-2 rounded-2xl border border-white/22 bg-white/8 px-5 py-3 font-bold text-white">
              اعمل سلطتك <ChevronLeft className="h-4 w-4" />
            </Link>
          </div>
        </div>
        <div className="relative overflow-hidden rounded-[38px] bg-white p-4 shadow-soft">
          <SmartImage src={heroImage} alt="طبق سلطة فريش" width={1200} height={900} priority sizes="(max-width: 768px) 100vw, 42vw" className="h-[440px] w-full rounded-[30px]" />
          <div className="absolute inset-x-4 bottom-4 rounded-[28px] bg-white/92 p-4 shadow-elevated">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <div className="font-bold text-[#123022]">خصم ٢٠٪ على أول طلب</div>
              </div>
      ...
      <section className="mt-8 rounded-[32px] border border-white/70 bg-white/92 p-6 shadow-soft">
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_auto]">
          <div className="rounded-2xl border border-slate-200 bg-[#fbfaf6] px-4 py-4 text-slate-700">
            ...
            <div className="mt-1 font-semibold">{settings.address}</div>
          </div>
      ...
      <section className="mt-8">
        <SectionTitle title="التصنيفات" subtitle="اختصار سريع يوصلك للمنيو الصح" />
